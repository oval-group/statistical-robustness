...
from convex_adversarial.dual_inputs import select_input
from convex_adversarial.dual_layers import select_layer
...
class DualNetwork(nn.Module):   
    def __init__(self, net, X, epsilon, 
        ...
        #print('before creating DualNetwork')
        #input()
        ...
        for i,(in_f,out_f,layer) in enumerate(zip(nf[:-1], nf[1:], net)): 
            ...
            if i < len(net)-1: 
                ...
                self.last_layer = dual_layer  #.cuda()

        #print('after creating DualNetwork')
        #input()

        #[d.cuda() for d in dual_net]

        #self.dual_net2 = nn.ModuleList(dual_net)
    ...
    def forward(self, c):
        ...
        for l in reversed(self.dual_net[1:]):
            #print(type(l), l.parameters())
            #print([p.device for p in l.parameters()])
...
class RobustBounds(nn.Module): 
    ...
    def forward(self, X,y): 
        ...
        #print('before calling .cuda() on DualNetwork')
        #input()
        #dual.cuda()
        #print('after calling .cuda() on DualNetwork')
        #input()
        ...
        #c = Variable(torch.eye(num_classes)[y].unsqueeze(1) - torch.eye(num_classes).unsqueeze(0))
        if X.is_cuda:
            c = c.cuda()
        f = -dual(c)
        return f.cuda()
